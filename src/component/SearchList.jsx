...
set
  ...
  return (
    <div>
    <input type='text' onChange={searchChange} placeholder='enter to search for what you are looking for' style={{padding:"20px", borderRadius:"30px", width:"500px", fontSize:20}}/>
 
 <div>{Data.filter((search)=> search.price.toLowerCase().includes(userSearch)).map((val)=>{
        const {id,title, category, price, img, desc} = val;
    return(
        <div key={id} style={{width:"500px",}}>
        <div style={{borderRadius:"20px"}}>
            <img style={{height:"200px",padding:"20px", borderRadius:"20px", backgroundPosition:"center", backgroundSize:"contain"}} src={img} alt={title} />
        </div>
        ...
        <div style={{textAlign:'center', width:"100%"}}>{desc}</div>
        </div>
        </div>
    )
 })}</div>
